...
public class RegionCommand extends SimpleCommand {
	...
	private enum TaskType {
		...
		/**
		 * @deprecated Will become obsolete once the book-config feature is
		 * implemented.
		 */
		@Deprecated
		USE_META_DATA;
	...
	private void schudleFindContainerBlocks(Player player, TaskType task, String group, String filter) throws CommandException {
		...
		World world = BukkitAdapter.adapt(sel.getWorld());
		...
		manager.getPlugin().getServer().getScheduler().runTaskAsynchronously(manager.getPlugin(), new FindContainers(manager, player, world, min, max, task, group, filterList));
	}
	...
	private class FindContainers implements Runnable {
		...
		public FindContainers(TreasureManager manager, Player player, World world, BlockVector3 min, BlockVector3 max, TaskType task, String group, Set<Material> filterList) {
			this.manager = manager;
			this.player = player;
			this.world = world;
			this.min = min;
			...
			for (y = min.getBlockY(); y < max.getBlockY(); ++y) {
				for (z = min.getBlockZ(); z < max.getBlockZ(); ++z) {
					for (x = min.getBlockX(); x < max.getBlockX(); ++x) {
						...
						for(Material checkMaterial : filterList) {
							if (type == checkMaterial) {
								found.add(b.getLocation());
								break;
							}
						}
					}
